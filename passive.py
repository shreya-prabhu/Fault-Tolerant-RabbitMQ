...
class PassiveServer:
    def __init__(self):
        receive_from_clients = threading.Thread(target=self.messages_from_clients)
        receive_from_clients.daemon = True
        receive_from_clients.start()

        receive_from_active = threading.Thread(target=self.messages_from_active)
